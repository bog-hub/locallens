...
async function getProfileData(userId: string) {
  ...
  // Guard: if userId is not a valid ObjectId, bail early
  ...
  // Fetch bookmarked businesses separately
  const bookmarkedUser = await User.findById(userId)
    .populate({
      path:   'bookmarks',
      model:  'Business',
  ...
  return {
    ...
    bookmarks: JSON.parse(JSON.stringify((bookmarkedUser as any)?.bookmarks ?? [])),
  };
}
...
export default async function ProfilePage() {
  ...
  // If session has a stale/invalid ID, sign them out gracefully
  if (!data) {
        ...
        <section>
          ...
          {reviews.length === 0 ? (
            <div className="card text-center py-12 text-gray-400">
              ...
              <Link href="/businesses" className="btn-primary text-sm mt-4 inline-flex">
                Discover businesses
              </Link>
            </div>
          ) : (
